...
def ordering(request):
    ...
    if is_user_registered(session_key):
        ...
        context['products'] = ProductInBasket.objects.filter(
    ...
    if request.method == 'POST':
        ...
        if is_user_registered(session_key):
            Orders.objects.create(session_key=None, user = is_user_registered(session_key), order = order)
            ProductInBasket.objects.filter(user = is_user_registered(session_key)).delete()
        else:
